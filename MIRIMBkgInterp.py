...
class MIRIMBkgInterp():
    """
    This class attempts to compute a reasonable background estimation for MIRI Imager data. It will first interpolate a
    background for a user defined aperture around a given source coordinate, in order to mask out the source. It will
    then attempt one of 3 background fitting routines, and return both the background subtracted image, and the
    ...
    bkg_mode : str
        Type of background image to compute from the masked input images ("simple", "polynomial" or "None").
    ...
    kernel: ndarray
        A 2D numpy array of size (3,3) to use in convolving the masked background image. If set to "None" the masked
        background image will not be convolved.
    combine_fits : bool
    ...
    def polynomial_bkg(self, data, v_wht=1., h_wht=1., degree=3):
        ...
        for z in range(0, k):
            ...
            # Compute average of both images to get final background.
    ...
    def simple_median_bkg(self, data, v_wht=1., h_wht=1.):
        ...
        ----------
        data : ndarray
            The 3D array containing the masked image data.
        v_wht, h_wht : float
        ...
        -------
        bkg : ndarray
            The 3D simple background image.
        """
    ...
    def run(self, data):
        ...
        -------
        diff: ndarray
            A 3D background subtracted image.
        bkg: ndarray
            A 3D interpolated background image.
        """
        self.print_inputs()
...
class MIRIMBkgInterp():
    ...
    def simple_median_bkg(self, data, v_wht=1., h_wht=1.):
        ...
        ----------
        data : ndarray
            The 3D array containing the masked image.
        v_wht, h_wht : float
            Weights for the row and column medians. Default is 1.
        ...
        for z in range(0, k):
            ...
            # Calculate the weighted average of both median images to get final background
            bkg_avg = np.average([bkg_v, bkg_h], weights=[v_wht, h_wht], axis=0)
            bkg[z] = bkg_avg
    ...
    def run(self, data):
        """
        Masks out source and interpolates the background in each slice.
